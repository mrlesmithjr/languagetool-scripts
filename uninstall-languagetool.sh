...
# Dry-run flag (for testing without deletion)
DRY_RUN=false
...
# Confirm uninstallation unless --force flag is provided
...
if [ "$DRY_RUN" = true ]; then
  echo "Dry-run mode: Removing launchd service (skipped)"
else
  if [ -f "$PLIST" ]; then
    rm -f "$PLIST"
...
if [ "$DRY_RUN" = true ]; then
  echo "Dry-run mode: Removing LanguageTool files (skipped)"
else
